---
title: "Relative Abundance Heatmap_non decon"
output:
...
```{r}
metadata <- read.csv("C:/BoxDrive/Box/Science/Fisheries/Projects/eDNA/Filter Pore Size Experiment/github/eDNA_FilterPoreSize/metadata/NPFPS_metadata.csv")

```

#data non decontaminated
```{r}
data_nondecon <- read_xlsx("C:/BoxDrive/Box/Science/Fisheries/Projects/eDNA/Filter Pore Size Experiment/github/eDNA_FilterPoreSize/ninigret_pond_data/results/Results_matrix_relative.xlsx")
...
#non decontaminated data
df_annotated_nondecon <- data_nondecon %>% gather("GMGI.Sample.ID", "Rel_Ab", 4:last_col()) %>%
...
#data contaminated
```{r}
data_decon <- read_xlsx("C:/BoxDrive/Box/Science/Fisheries/Projects/eDNA/Filter Pore Size Experiment/github/eDNA_FilterPoreSize/ninigret_pond_data/results_decon/Results_matrix_relative.xlsx")
...
#non decontaminated data
df_annotated_decon <- data_decon %>% gather("GMGI.Sample.ID", "Rel_Ab", 5:last_col()) %>%
  left_join(., taxlevels, by = "Species_name") %>%
  left_join(., metadata, by = "GMGI.Sample.ID")
...
commonNames_annotated <- read_xlsx("C:/BoxDrive/Box/Science/Fisheries/Projects/eDNA/Filter Pore Size Experiment/github/eDNA_FilterPoreSize/ninigret_pond_data/Taxonomic_assignments/CommonNames_required_edited.xlsx")
...
df_annotated <- df_annotated_nondecon %>%
...
## tax list non decon
df_tax_nondecon <- df_annotated_nondecon %>% dplyr::select(Species_name, Kingdom:Genus) %>% distinct()
...
for (i in commonNames_annotated$Species_name) {
  ...
  # Apply filter based on the current row's condition
df_annotated <- df_annotated_decon %>%
  mutate(across(c(Common_name, Category, Kingdom, Phylum, Class, Order, Family, Genus, species, PhylogenySorting),
                ~case_when(Species_name == current_row$Species_name ~ current_row[[cur_column()]],
                           TRUE ~ .x)))
}
...
## tax list non decon
df_tax_decon <- df_annotated_decon %>% dplyr::select(Species_name, Kingdom:Genus) %>% distinct()
...
```{r}
df_filtered_nondecon <- df_annotated_nondecon %>% 
  filter(!Category == "Other" & !Category == "Livestock" & !Category == "Unassigned" & !Category == "Human") 
```


```{r}
df_filtered_decon <- df_annotated_decon %>% 
  filter(!Category == "Other" & !Category == "Livestock" & !Category == "Unassigned" & !Category == "Human") 
```
...
df_annotated_decon %>%
  ...
  ggplot(., aes(x = GMGI.Sample.ID, y = Common_name)) +
...
ggsave("C:/BoxDrive/Box/Science/Fisheries/Projects/eDNA/Filter Pore Size Experiment/github/eDNA_FilterPoreSize/ninigret_pond_data/figures_decon/Relative_abundance_blanks.png", width = 22, height = 14)  
```
