...
std_curve %>% 
  ...
  geom_text(aes(x = log10(LOD_threshold), y = max(std_curve$Cq, na.rm=TRUE) * 0.3, label = "LOD"), 
  ...
  geom_text(aes(x = log10(LOQ_threshold), y = max(std_curve$Cq, na.rm=TRUE) * 0.3, label = "LOQ"), 
            angle = 90, vjust = -0.5, hjust = -0.1, color = "grey40", size = 4) +

  theme_bw() +
  
  labs(
    x = "log10 of starting quantity",
    y = "Cq"
  )
```
