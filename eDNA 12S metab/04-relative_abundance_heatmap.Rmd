...
## Sankey plot at taxonomic levels 
...
sankey_df <- raw_df %>% 
   mutate(sum = sum*1000000) %>% group_by(Class, Order) %>% reframe(reads = sum(sum)) #%>%
  # mutate(Order = factor(Order, levels = unique(Order[order(Class, desc(Order))]))) %>%
  # make_long(Class, Order, value = reads)

edges2 <- sankey_df %>% dplyr::select(from = Class, to=Order)
number_of_classes <- length(unique(edges2$from))
number_within_classes <- edges2 %>% group_by(from) %>% reframe(count = n_distinct(to)) %>% dplyr::select(count)

name2 <- unique(c(as.character(edges2$from), as.character(edges2$to)))

vertices2 <- data.frame(
  name=c("origin", name2),
  group=c( rep(NA,(number_of_classes+1)), ## origin + NA for each class 
           rep( paste("group", seq(1,number_of_classes), sep=""), ## group1-number of classes (e.g., 1-6)
                each=number_within_classes)) ## the number within each class 
 # cluster=sample(letters[1:4], length(name2), replace=T),
#  value=sample(seq(10,30), length(name2), replace=T)
)

mygraph <- graph_from_data_frame(edges, vertices=sankey_df)
...
ggsave("example_output/Figures/Sankeyplot.png", width=8, height=10)
```
